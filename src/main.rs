...
fn get_character_images(
  tile_size: u32,
  font: &Font<'static>,
) -> Result<Vec<(char, BaseImage)>, io::Error> {
  Ok(
    (b'a'..=b'z')
      .chain(b'A'..=b'Z')
      .map(|c| c as char)
      ...
      .map(|c| {
        (
          c,
          get_character_image(tile_size, &font, &Rgb(BACKGROUND), &Rgb(FOREGROUND), &c),
        )
      })
      .collect(),
  )
...
fn get_characters(
  ...
  (0..source_gray.dimensions().0 / tile_size)
    .cartesian_product(0..source_gray.dimensions().1 / tile_size)
    .par_bridge()
    .map(|(x, y)| {
      let source_gray_tile = SubImage::new(
        source_gray,
        x * tile_size,
        y * tile_size,
        tile_size,
...
fn fill_canvas(
  ...
  get_characters(source_gray, characters, tile_size)
    ...
    .par_bridge()
    .map(|(x, y, character)| {
      let source_rgb_tile = SubImage::new(
        source_rgb,
        x * tile_size,
        y * tile_size,
        tile_size,
      ...
      let character_image = get_character_image(
        original_tile_size,
        font,
        &get_average_britghness(&source_rgb_tile),
        &get_average_color(&source_rgb_tile),
        character,
      );

      (*x, *y, character_image)
    })
    .collect::<Vec<(u32, u32, BaseImage)>>()
    .iter()
...
fn compare_images(a: &BaseImage, b: &SubImage<&BaseImage>) -> f32 {
  ...
  (0..a.dimensions().0)
    .cartesian_product(0..a.dimensions().1)
    .fold(0., |acc, (x, y)| {
...
fn best_character<'a>(
  source_tile: &SubImage<&BaseImage>,
